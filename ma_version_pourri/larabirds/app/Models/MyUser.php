...
class MyUser
{
	...
	public function __construct( string $login, string $password = null, string $name, string $lastname)
	{
		$this->setLogin($login);
	...
	{
		$request = DB::connection()->getPdo()->prepare('INSERT INTO '.self::USER_TABLE.'(login,password,name,lastname) VALUES (:login,:password,:name,:lastname)');
		$ok =  $request->bindValue( ":login", $this->_login, PDO::PARAM_STR );
